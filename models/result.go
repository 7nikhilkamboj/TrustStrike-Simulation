...
// successfully sent to the remote provider
...
func (r *Result) UpdateGeo(addr string) error {
	...
	if err != nil {
		log.Errorf("Error opening MaxMind DB: %v", err)
		return err
	}
	defer mmdb.Close()
	ip := net.ParseIP(addr)

	// Fallback for local/private IPs to allow markers during testing
	if ip.IsLoopback() || isPrivateIP(ip) {
		r.IP = addr
		// Default to a central location (SF) for testing purposes
		r.Latitude = 37.7749
	...
	if err != nil {
		log.Errorf("Error looking up IP %s: %v", addr, err)
		return err
	}
	...
	if r.Latitude == 0 && r.Longitude == 0 {
		log.Debugf("No coordinates found for IP: %s", addr)
	}
	return db.Save(r).Error
}
