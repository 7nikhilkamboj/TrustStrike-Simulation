...
func (s *ModelsSuite) TestMailLogGetTo(ch *check.C) {
	campaign := s.createCampaignDependencies(ch)
	// Add a target with a name
	t1 := Target{
	...
	campaign.Groups = []Group{g}
	ch.Assert(PostCampaign(&campaign, campaign.UserId), check.Equals, nil)

	m := &MailLog{}
	err := db.Where("campaign_id=?", campaign.Id).Find(m).Error
	ch.Assert(err, check.Equals, nil)

	// GetTo should return ONLY the email address
	ch.Assert(m.GetTo(), check.DeepEquals, []string{"test1@example.com"})
}
