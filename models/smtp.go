...
func PutSMTP(s *SMTP) error {
	// Verify that the user has permission to modify this SMTP profile
	existing, err := GetSMTP(s.Id, s.UserId)
	if err != nil {
		return err
	}
...
func DeleteSMTP(id int64, uid int64) error {
	// Verify that the user has permission to delete this SMTP profile
	s, err := GetSMTP(id, uid)
	if err != nil {
		return err
	}
	...
	err = db.Delete(SMTP{Id: id}).Error
	if err != nil {
		log.Error(err)
	}
	return err
