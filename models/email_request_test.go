...
type emailWriterTo struct {
	*email.Email
}

func (e *emailWriterTo) WriteTo(w io.Writer) (int64, error) {
	b, err := e.Bytes()
	if err != nil {
		return 0, err
	}
...
func (s *ModelsSuite) TestEmailRequestGenerate(ch *check.C) {
	...
	_, err = (&emailWriterTo{msg}).WriteTo(msgBuff)
	ch.Assert(err, check.Equals, nil)
...
func (s *ModelsSuite) TestEmailRequestURLTemplating(ch *check.C) {
	...
	ch.Assert(string(got.Text), check.Equals, string(expectedURL))
	ch.Assert(string(got.HTML), check.Equals, string(expectedURL))
}
...
func (s *ModelsSuite) TestEmailRequestGetTo(ch *check.C) {
	req := &EmailRequest{
		BaseRecipient: BaseRecipient{
	...
	// GetTo should return ONLY the email address
	ch.Assert(req.GetTo(), check.DeepEquals, []string{"test@example.com"})
}
