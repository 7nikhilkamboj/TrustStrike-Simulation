...
trust_strike implements simple Role-Based-Access-Control (RBAC) to control access to
certain resources.

By default, go-gomailtrike has two separate roles, with each user being assigned to
a single role:
...
const (
	// RoleAdmin is used for go-gomailtrike system administrators. Users with this
	// role have the ability to manage all objects within go-gomailtrike, as well as
	// system-level configuration, such as users and URLs.
	RoleAdmin = "admin"
	// RoleUser is used for standard go-gomailtrike users. Users with this role can
	// create, manage, and view go-gomailtrike objects and campaigns.
	RoleUser = "user"

	// PermissionViewObjects determines if a role can view standard go-gomailtrike
	...
	// standard go-gomailtrike objects.
...
// Role represents a user role within go-gomailtrike. Each user has a single role
