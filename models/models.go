...
// randomly
const InitialAdminPassword = "trust_strike_INITIAL_ADMIN_PASSWORD"
...
// randomly
const InitialAdminApiToken = "trust_strike_INITIAL_ADMIN_API_TOKEN"
...
func Setup(c *config.Config) error {
	...
	// If this is the first time the user is installing trust_strike, then we will
	// generate a temporary password for the admin user.
	//
	// We do this here instead of in the block above where the admin is created
	// since there's the chance the user executes trust_strike and has some kind of
	...
	// Initialize token blacklist checker for auth package
	auth.BlacklistChecker = IsTokenBlacklisted
	auth.Blacklister = BlacklistToken
