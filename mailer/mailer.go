...
type Mail interface {
	...
	Success() error
	Generate(msg *email.Email) error
...
func sendMail(ctx context.Context, dialer Dialer, ms []Mail) {
	...
	for i, m := range ms {
		...
		// Reset message headers and body
		message.Headers = textproto.MIMEHeader{}
		message.Text = nil
		message.HTML = nil
		...
		err = sender.Send(smtp_from, m.GetTo(), &emailWriterTo{message})
		if err != nil {
