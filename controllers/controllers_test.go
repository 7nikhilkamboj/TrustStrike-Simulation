...
func setupTest(t *testing.T) *testContext {
	...
	ctx := &testContext{}
	ctx.config = conf
	ctx.adminServer = httptest.NewUnstartedServer(NewAdminServer(ctx.config.AdminConf, ctx.config.Keycloak).server.Handler)
	...
	u, err := models.GetUserByUsername(models.DefaultAdminUsername)
	if err != nil {
		t.Fatalf("error getting admin user from database: %v", err)
	}
	hash, err := auth.GeneratePasswordHash("trust_strike")
	if err != nil {
