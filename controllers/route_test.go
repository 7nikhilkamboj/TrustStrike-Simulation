...
func attemptLogin(t *testing.T, ctx *testContext, client *http.Client, username, password, optionalPath string) *http.Response {
	...
	// If we have a successful login, we expect a trust_strike_jwt cookie
	if resp.StatusCode == http.StatusOK || resp.StatusCode == http.StatusFound {
		found := false
		for _, cookie := range resp.Cookies() {
			if cookie.Name == "trust_strike_jwt" {
				found = true
				break
			}
		}
		if !found {
			t.Log("trust_strike_jwt cookie not found in response")
		}
	}
...
func TestSuccessfulLogin(t *testing.T) {
	ctx := setupTest(t)
	defer tearDown(t, ctx)
	resp := attemptLogin(t, ctx, nil, "admin", "trust_strike", "")
	got := resp.StatusCode
	expected := http.StatusOK
	if got != expected {
		t.Fatalf("invalid status code received. expected %d got %d", expected, got)
...
func TestSuccessfulRedirect(t *testing.T) {
	...
	resp := attemptLogin(t, ctx, client, "admin", "trust_strike", fmt.Sprintf("?next=%s", next))
	got := resp.StatusCode
...
func TestAccountLocked(t *testing.T) {
	ctx := setupTest(t)
	defer tearDown(t, ctx)
	resp := attemptLogin(t, ctx, nil, "houdini", "trust_strike", "")
	got := resp.StatusCode
	expected := http.StatusUnauthorized
	if got != expected {
		t.Fatalf("invalid status code received. expected %d got %d", expected, got)
