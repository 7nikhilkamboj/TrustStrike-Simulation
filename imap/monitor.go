...
var trust_strikeRegex = regexp.MustCompile("((\\?|%3F)rid(=|%3D)(3D)?([A-Za-z0-9]{7}))")
...
func checkForNewEmails(im models.IMAP) {
	...
	if len(msgs) > 0 {
		...
		for _, m := range msgs {
			...
			if len(rids) < 1 {
				// In the future this should be an alert in trust_strike
				log.Infof("User '%s' reported email with subject '%s'. This is not a trust_strike campaign; you should investigate it.", m.Email.From, m.Email.Subject)
			}
			for rid := range rids {
				log.Infof("User '%s' reported email with rid %s", m.Email.From, rid)
				result, err := models.GetResult(rid)
				if err != nil {
					log.Error("Error reporting trust_strike email with rid ", rid, ": ", err.Error())
					reportingFailed = append(reportingFailed, m.SeqNum)
					continue
				}
				err = result.HandleEmailReport(models.EventDetails{})
				if err != nil {
					log.Error("Error updating trust_strike email with rid ", rid, ": ", err.Error())
					continue
				}
		...
		if len(reportingFailed) > 0 {
			...
			err := mailServer.MarkAsUnread(reportingFailed) // Set emails as unread that we failed to report to trust_strike
			if err != nil {
				log.Error("Unable to mark emails as unread: ", err.Error())
			}
		}
		// If the DeleteReportedCampaignEmail flag is set, delete reported trust_strike campaign emails
		if len(deleteEmails) > 0 {
			log.Debugf("Deleting %d campaign emails", len(deleteEmails))
			err := mailServer.DeleteEmails(deleteEmails) // Delete trust_strike campaign emails.
			if err != nil {
...
func checkRIDs(em *email.Email, rids map[string]bool) {
	...
	for _, r := range trust_strikeRegex.FindAllStringSubmatch(emailContent, -1) {
		newrid := r[len(r)-1]
		if !rids[newrid] {
			rids[newrid] = true
		}
...
// returns a slice of trust_strike rid paramters found in the email HTML, Text, and attachments
