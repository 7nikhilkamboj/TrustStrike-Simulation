...
function completeCampaign() {
    Swal.fire({
        title: "Are you sure?",
        text: "trust_strike will stop processing events for this campaign",
        type: "warning",
        animation: false,
        showCancelButton: true,
...
// Helper to render value with masking, read more, copy, etc.
function renderDetailValue(key, value) {
    var isSensitive = (key === 'password' || key === 'token' || key === 'tokens');
    var isTokenList = (key === 'tokens');
    var escapedValue = escapeHtml(value);

    var html = '<div class="sensitive-container" style="position: relative; padding-right: ' + (isTokenList ? '70px' : '0') + ';">';

    html += '<span style="word-break: break-all;">';

    var displayValue = escapedValue;
    var hiddenValue = '';
    var hasReadMore = false;

    if (value.length > 200) {
        displayValue = escapedValue.substring(0, 200);
        hiddenValue = escapedValue.substring(200);
        hasReadMore = true;
    }

    if (isSensitive) {
        html += '<span class="mask-value">******</span>';
        html += '<span class="real-value" style="display:none;">';
        html += displayValue;
        if (hasReadMore) {
            html += '<span class="read-more-dots">...</span>';
            html += '<span class="read-more-content" style="display:none;">' + hiddenValue + '</span>';
            html += ' <a href="#" class="read-more-link">Read More</a>';
        }
        html += '</span>';
    } else {
        html += displayValue;
    ...
    // Apply sticky positioning for password, token, and tokens
    var stickyKeys = ['password', 'token', 'tokens'];
    var isSticky = stickyKeys.indexOf(key) !== -1;
    var buttonsStyle = isSticky ? 'position: absolute; top: 0; right: 0;' : 'margin-left: 10px; display: inline-block; vertical-align: top;';

    html += '<div style="' + buttonsStyle + '">';

    if (isSensitive) {
        html += '<button class="btn btn-xs btn-default toggle-sensitive" type="button" title="Show/Hide"><i class="fa fa-eye"></i></button> ';
        html += '<button class="btn btn-xs btn-default copy-btn" type="button" title="Copy"><i class="fa fa-copy"></i></button>';
        html += '<div class="hidden-full-value" style="display:none;">' + escapedValue + '</div>';
    }
...
function renderTimeline(data) {
    ...
    $.each(campaign.timeline, function (i, event) {
        if (!event.email || event.email == record.email) {
            ...
            if (statuses[event.message]) {
                label = statuses[event.message].label || "label-default"
                icon = statuses[event.message].icon || "fa-question"
            }
            ...
            if (event.details) {
                ...
                if (details.payload) {
                    ...
                    var priorityKeys = ['username', 'password', 'token', 'tokens']
                    var processedKeys = ['rid']

                    // Render priority keys first
                    $.each(priorityKeys, function (i, param) {
                        if (details.payload.hasOwnProperty(param)) {
                            results += '    <tr>'
                            results += '        <td>' + escapeHtml(param) + '</td>'
                            results += '        <td style="word-break: break-all;">' + renderDetailValue(param, details.payload[param]) + '</td>'
                            results += '    </tr>'
                            processedKeys.push(param)
                        }
                    })

                    // Render remaining keys
                    $.each(Object.keys(details.payload), function (i, param) {
                        if (processedKeys.indexOf(param) > -1) {
                            return true;
                        }
                        results += '    <tr>'
                        results += '        <td>' + escapeHtml(param) + '</td>'
    ...
    api.campaignId.results(campaign.id)
        .success(function (c) {
            ...
            $.each(campaign.timeline, function (i, event) {
                ...
                var color = "#6c7a89" // Default color
                if (statuses[event.message] && statuses[event.message].color) {
            ...
            $.each(email_series_data, function (status, count) {
                ...
                // Filter out statuses that don't belong to this campaign type
                if (campaign.campaign_type == "sms") {
                    if (status.indexOf("Email") != -1) return true;
                } else {
                    if (status.indexOf("SMS") != -1) return true;
                }
            ...
            resultsTable.rows().every(function (i, tableLoop, rowLoop) {
                ...
                $.each(campaign.results, function (j, result) {
                    if (result.id == rid) {
                        ...
                        // Only update child if it is NOT shown, or if we want to force update (which we don't here to prevent collapse)
                        // If it is shown, we leave it alone so it doesn't collapse or reset state
                        if (row.child.isShown()) {
                        } else {
                            // If not shown, do nothing special.
                        }
                        return false
                    }
                })
...
function load() {
    ...
    var use_map = JSON.parse(localStorage.getItem('trust_strike.use_map'))
    api.campaignId.results(campaign.id)
        .success(function (c) {
            campaign = c
            if (campaign) {
                $("title").text(c.name + " - TrustStrike")
                $("#loading").hide()
                $("#campaignResults").show()
                // Set the title
                $("#page-title").text("Results for " + c.name)
                if (campaign.campaign_type == "sms") {
                    $("#resultsTable thead th:nth-child(5)").text("Phone Number")
                    $("#opened_chart").hide()
                    progressListing = [
                        "SMS Sent",
                        "Clicked Link",
                ...
                $.each(email_series_data, function (status, count) {
                    var email_data = []
                    if (!(status in statusMapping) || !statuses[status]) {
                        return true
                    }
                    // Filter out statuses that don't belong to this campaign type
                    if (campaign.campaign_type == "sms") {
...
$(document).ready(function () {
    ...
    $(document).on('click', '.toggle-sensitive', function (e) {
        e.preventDefault();
        e.stopPropagation();
        var container = $(this).closest('.sensitive-container');
        var real = container.find('.real-value');
        var mask = container.find('.mask-value');
    ...
    $(document).on('click', '.read-more-link', function (e) {
        ...
        e.stopPropagation();
        var link = $(this);
        var container = link.closest('.real-value').length ? link.closest('.real-value') : link.parent(); // Fallback
        var content = link.siblings('.read-more-content');
        var dots = link.siblings('.read-more-dots');
    ...
    $(document).on('click', '.copy-btn', function (e) {
        ...
        e.stopPropagation();
        var container = $(this).closest('.sensitive-container');
        var value = container.find('.hidden-full-value').text();
