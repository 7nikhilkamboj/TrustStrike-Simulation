...
//
// By default, since trust_strike anticipates connections originating to hosts on
// the local network, we only deny access to the link-local addresses at
// 169.254.0.0/16.
//
// If hosts are provided, then go-gomailtrike blocks access to all local addresses
